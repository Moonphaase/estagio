...
def validate_file_extension(file):
    ...
    if ext not in ALLOWED_EXTENSIONS:
        raise ValueError(f"Extensão não permitida: {ext}. Permitidas: {', '.join(ALLOWED_EXTENSIONS)}")
    return ext
...
def validate_file_size(file, max_size=MAX_FILE_SIZE):
    if file.size > max_size:
        raise ValueError(f"Ficheiro demasiado grande. Máximo: {max_size // (1024*1024)}MB")
